...
angular.module('diff-match-patch', [])
	.factory('dmp', ['$window', function($window) {
		...
		function createHtmlFromDiffs(diffs, display, options, print) {
			var pattern_amp = /&/g;
			var pattern_lt = /</g;
			var pattern_gt = />/g;
			...
			for (var x = 0; x < diffs.length; x++) {
				var op = diffs[x][0];
				if (print === displayType.LEFT && op === 1) {
					continue;
				}
				if(print === displayType.RIGHT && op === -1) {
					continue;
				}
				var text = diffs[x][1];
		...
		return {
			createDiffHtml: function(left, right, options, print) {
				if (assertArgumentsIsStrings(left, right)) {
					var dmp = new diff_match_patch();
					var diffs = dmp.diff_main(left, right);
					return createHtmlFromDiffs(diffs, displayType.INSDEL, options, print);
				} else {
					return '';
				}
			},

			createProcessingDiffHtml: function(left, right, options, print) {
				if (assertArgumentsIsStrings(left, right)) {
					var dmp = new diff_match_patch();
					var diffs = dmp.diff_main(left, right);
					//dmp.Diff_EditCost = 4;
			...
			createSemanticDiffHtml: function(left, right, options, print) {
				if (assertArgumentsIsStrings(left, right)) {
					var dmp = new diff_match_patch();
					var diffs = dmp.diff_main(left, right);
			...
			createLineDiffHtml: function(left, right, options, print) {
				if (assertArgumentsIsStrings(left, right)) {
					var dmp = new diff_match_patch();
					var a = dmp.diff_linesToChars_(left, right);
					var diffs = dmp.diff_main(a.chars1, a.chars2, false);
					dmp.diff_charsToLines_(diffs, a.lineArray);
					return createHtmlFromDiffs(diffs, displayType.LINEDIFF, options, print);
				} else {
					return '';
				}
			}
		...
		var ddo = {
				...
				link: function postLink(scope, iElement) {
						var listener = function() {
							iElement.html(dmp.createDiffHtml(scope.left, scope.right, scope.options, scope.print));
	...
	.directive('processingDiff', ['$compile', 'dmp', function factory($compile, dmp) {
		var ddo = {
				...
				link: function postLink(scope, iElement) {
						var listener = function() {
							iElement.html(dmp.createProcessingDiffHtml(scope.left, scope.right, scope.options, scope.print));
							$compile(iElement.contents())(scope);
						};
						scope.$watch('left', listener);
						scope.$watch('right', listener);
	...
	.directive('semanticDiff', ['$compile', 'dmp', function factory($compile, dmp) {
		var ddo = {
				...
				link: function postLink(scope, iElement) {
						var listener = function() {
							iElement.html(dmp.createSemanticDiffHtml(scope.left, scope.right, scope.options, scope.print));
							$compile(iElement.contents())(scope);
						};
						scope.$watch('left', listener);
						scope.$watch('right', listener);
	...
	.directive('lineDiff', ['$compile', 'dmp', function factory($compile, dmp) {
		var ddo = {
				...
				link: function postLink(scope, iElement) {
					var listener = function() {
						iElement.html(dmp.createLineDiffHtml(scope.left, scope.right, scope.options, scope.print));
						$compile(iElement.contents())(scope);
					};
					scope.$watch('left', listener);
					scope.$watch('right', listener);
